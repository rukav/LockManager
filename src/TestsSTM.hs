module Main where
import Control.Concurrent.LockManagerSTM
...
sameTidItem l r = do
  lm <- mkLockManager hashInt
  let (item, tid, timeout) = (1, 1, 0)
...
test_trans100 = do
  tm <- make
  lm <- mkLockManager hashInt
  let (item, timeout, trans) = (1, 2, 100)
